function [I,C] = inpaint(I,M,S,koff,CUT,Schet1,poisk)
...
[aa bb]=size(M);
% Lbp(1:aa,1:bb)=1000000;
I_gray=rgb2gray(I.rgb);
% mapping=getmapping(8,'u2'); 
% H1=lbp(I_gray,1,8,mapping); 
% Lbp(2:aa-1,2:bb-1)=H1;

% Define inpainting parameters.
pSize       = 5;     % half-size of exemplar patches
...
% Lbp = padarray(Lbp,pSize*[1 1],NaN,'both');
...
%    block1=block;
  ...
  Raz_x=B.boundary(blockIndex,1)+pIndex;
  Raz_y=B.boundary(blockIndex,2)+pIndex;      
      ...
      %%%%%%EBM
      if poisk==0
   match = findmatch(double(I.rgb),double(S),double(block),mask,frame,Raz_x,Raz_y, koff,CUT);
   ...
   I.gray(B.boundary(blockIndex,1)+pIndex,...
      ...
      %%%%%%Markov field
     if poisk==1 
   match3 = findmatch_M(double(I.rgb),double(S),double(block),mask,frame,Raz_x,Raz_y,CUT);
      % Assign masked pixels using exemplar.
   I.rgb(B.boundary(blockIndex,1)+pIndex,...
         B.boundary(blockIndex,2)+pIndex,:) = match3;
     end
     
      %%%%%%STD
     if poisk==2
   match3 = findmatch_std(double(I.rgb),double(S),double(block),mask,frame,Raz_x,Raz_y,CUT);
      % Assign masked pixels using exemplar.
   I.rgb(B.boundary(blockIndex,1)+pIndex,...
         B.boundary(blockIndex,2)+pIndex,:) = match3;
%      figure, imshow(I.rgb)
   ...
   I.gray(B.boundary(blockIndex,1)+pIndex,...
          B.boundary(blockIndex,2)+pIndex) = mean(match3,3);
     end
